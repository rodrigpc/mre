...
TCPIPServer.setPort(gServer, 10000)
TCPIPServer.setFraming(gServer, '', '', '' ,'') -- STX/ETX framing for transmit and receive
...
function gHandleConnectionAccepted(con)
  ...
  TCPIPServer.Connection.transmit(con, 'Oi Mundo!!!')
  TCPIPServer.Connection.transmit(con, '\r\n')
  print('Enviei Ola mundo...')
end
...
function gHandleReceive(con, data)
  ...
  TCPIPServer.Connection.transmit(
    con,
    'Teste ok ' .. tostring(#data) .. ' bytes'
  )
  TCPIPServer.Connection.transmit(con, '\r\n')
  print('Respondi teste ok')
end
