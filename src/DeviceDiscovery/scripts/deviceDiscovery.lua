...
Script.serveFunction("DeviceDiscovery.setInterface","setInterface", "string")
...
--list of devices found by scanning
local foundDevices
...
function config()
  ...
  if(demoMode) then
    ...
    local success = Command.Scan.configure(deviceScanner, configMacAddress, configIpAddress, configSubnetMask, configDefaultGateway, configDhcpEnabled)
    scan()
    return success
  end
end
...
--]]
function setSelectionToConfig(selection)
...
--]]
function getDevicesJSON(devices)
  local scans = ""
  for key,value in pairs(devices) do
    scans = scans .. "{"
